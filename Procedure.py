    ...
    for pair in hubs_pair:#循环每一对城市对
        ...
        # 往返都有直达邮路且运量都大于0
            ...
            for z_1 in range(1,int(k_min)):
                for z_2 in range(int(k_min) - z_1):
                    for z_3 in range(int(k_min) - z_1 - z_2):
                        for x_1 in range(int(k_1) - z_1 - z_2 - z_3):
                            for x_2 in range(int(k_1) - z_1 - z_2 - z_3 - x_1):
                                for x_3 in range(int(k_1) - z_1 - z_2 - z_3 - x_1 - x_2):
                                    for y_1 in range(int(k_2) - z_1 - z_2 - z_3):
                                        for y_2 in range(int(k_2) - z_1 - z_2 - z_3 - y_1):
                                            for y_3 in range(int(k_2) - z_1 - z_2 - z_3 - y_1 - y_2):
            ...
            trans_cost = trans_cost.append([[pair[0], pair[1], final_truck_list_1, final_cost_1]])
            ...
            trans_cost = trans_cost.append([[pair[1], pair[0], final_truck_list_2, final_cost_2]])
    trans_cost = pd.DataFrame(np.array(trans_cost), columns=['收寄城市', '寄达城市', '车辆使用情况', '车辆成本'])
    return trans_cost
